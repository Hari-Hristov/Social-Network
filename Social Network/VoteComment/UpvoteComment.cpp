...
void UpvoteComment::execute(User*& currentUser, Post*& currentPost, Comment*& currentComment, const Vector<MyString>& input)
{
	...
	if (currentComment->hasBeenLikedBy(userId))
	{
		...
		currentUser->removePoint();
		std::cout << "Successfully removed the upvote." << std::endl;
	...
	else
	{
		...
		std::cout << "Successfully upvoted the comment." << std::endl;
